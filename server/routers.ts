...
export const appRouter = router({
  ...
  admin: router({
    ...
    approveAll: adminProcedure
      .input(z.object({ type: z.enum(["task", "item"]) }))
      .mutation(async ({ input }) => {
        return await adminApproveAll(input.type);
      }),
